...
class StackSingleton {
    static let sharedInstance = StackSingleton()
...
struct CoreDataStack {
    ...
    // MARK: Utils
    func addStoreCoordinator(storeType: String, configuration: String?, storeURL: URL, options : [AnyHashable: Any]?) throws {
...
extension CoreDataStack {
    ...
    func autoSave(delayInSeconds: Int) {
        ...
        if delayInSeconds > 0 {
            #if DEBUG
                print("CoredDataStack: autoSave: autosaving")
            #endif
            save()
            let delayInNanoSeconds = delayInSeconds * Int(NSEC_PER_SEC)

            //The following code was adopted from an answer on this Stack Overflow post. http://stackoverflow.com/questions/37801436/how-do-i-write-dispatch-after-gcd-in-swift-3
            let time = DispatchTime.now() + DispatchTimeInterval.nanoseconds(delayInNanoSeconds)
